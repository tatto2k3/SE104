...
export default function SearchPage() {
    ...
    const handleTicketClick = (flight) => {
        if (tripType === "roundTrip") {
            if (activeButton === "Depart") {
                ...
                setIsLoading(true);
                axios.get(`https://2b0c-113-161-73-175.ngrok-free.app/api/flight?FromLocation=${searchInfo.ToLocation}&ToLocation=${searchInfo.FromLocation}&DepatureDay=${searchInfo.ComeBackTime}`)
                    .then(res => {
    ...
    function queryAPI(depatureTime, arrivalTime) {
        if (searchInfo.FromLocation != null && searchInfo.ToLocation != null && searchInfo.DepartTime != null && searchInfo.ComeBackTime != null) {
            if (activeButton === "Depart") {
                setIsLoading(true);
                axios.get(`https://2b0c-113-161-73-175.ngrok-free.app/api/flight?FromLocation=${searchInfo.FromLocation}&ToLocation=${searchInfo.ToLocation}&DepartureTime=${depatureTime}&ArrivalTime=${arrivalTime}
                                &DepatureDay=${searchInfo.DepartTime}`)
                    .then(res => {
                        setSearchResult(res.data)
                        setIsLoading(false);
                ...
                setIsLoading(true);
                axios.get(`https://2b0c-113-161-73-175.ngrok-free.app/api/flight?FromLocation=${searchInfo.ToLocation}&ToLocation=${searchInfo.FromLocation}&DepartureTime=${depatureTime}&ArrivalTime=${arrivalTime}
