...
const SanBay = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch("https://2b0c-113-161-73-175.ngrok-free.app/api/sanbay/GetSanbays");
    ...
    const handleShowInfo = async () => {
        try {
            if (selectedSanbays.length > 0) {
                const response = await fetch(`https://2b0c-113-161-73-175.ngrok-free.app/api/sanbay/GetSanbayDetails?airportIds=${selectedSanbays.join(',')}`);
                const data = await response.json();
    ...
    const handleDelete = async () => {
        if (window.confirm("Bạn có chắc chắn xóa sân bay này?")) {
            try {
                const response = await axios.delete('https://2b0c-113-161-73-175.ngrok-free.app/api/sanbay', {
    ...
    const handleSearch = async () => {
        if (searchKeyword != "") {
            try {
                const response = await fetch(`https://2b0c-113-161-73-175.ngrok-free.app/api/sanbay/SearchSanbays?searchKeyword=${searchKeyword}`);
                const data = await response.json();
                setSanbays(data);
            } catch (error) {
